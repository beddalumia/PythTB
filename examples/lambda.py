...
#   how many steps to take along the path
path_steps=20
#   value of lambda interpolated from 0 to 1
all_lambda=np.linspace(0.0,1.0,path_steps,endpoint=True)
#   how many k-points to use
num_kpt=30
# two-dimensional wf_array in which we will store wavefunctions
# for all k-points and all values of lambda
...
for i_lambda in range(path_steps):
    ...
    # set of points in k-space (make sure last point is included)
    ...
    col=1.0-0.5*lmbd # color depending on value of lambda
    for band in range(eval.shape[0]):
        ax_band.plot(np.ravel(k_vals),eval[band,:],"-",color=[col,col,col])
...
fig_wann=pl.figure()
ax_wann=fig_wann.add_subplot(111)
...
# compute curvature
print "Berry curvature in k-lambda space: ",wf_kpt_lambda.berry_curv([0])
...
ax_band.set_xlabel("k-vector")
ax_band.set_ylabel("Band energies")
ax_band.set_xlim(-0.5,0.5)
fig_band.savefig("lambda_band.pdf")
...
fig_wann.savefig("lambda_wann.pdf")
